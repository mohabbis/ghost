...
Latest release in this repo is **v2.0.3** (Ghost 2.0 unified runtime). Published installers on GitHub may lag until the next release is cut — build from source for the newest behavior. macOS installers are Developer ID–signed and notarized when published; Windows installers may be unsigned (SmartScreen may warn). Verify downloads with [`SHA256SUMS.txt`](https://github.com/mohabbis/ghost/releases/latest/download/SHA256SUMS.txt). See `RELEASING.md`.
