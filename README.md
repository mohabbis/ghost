...
Latest release is [`v1.2.9`](https://github.com/mohabbis/ghost/releases/tag/v1.2.9). Ghost remains an early technical preview. macOS installers are Developer ID–signed and notarized; Windows installers are currently unsigned (SmartScreen may warn). Verify downloads with [`SHA256SUMS.txt`](https://github.com/mohabbis/ghost/releases/latest/download/SHA256SUMS.txt). See `RELEASING.md`.
