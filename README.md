...
Current builds are preview quality. The current app version is `v1.2.5`. macOS builds may be ad-hoc signed; if macOS blocks the app, approve it under **System Settings -> Privacy & Security**. Verify downloads with [`SHA256SUMS.txt`](https://github.com/mohabbis/ghost/releases/latest/download/SHA256SUMS.txt). Notarized releases require Apple signing secrets — see `RELEASING.md`.
