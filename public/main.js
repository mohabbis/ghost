...
function platformDetection() {
  ...
  if (platform === "mac" && sub) sub.textContent = "Detected macOS — v2.0.1 (notarized).";
  if (platform === "windows" && sub) sub.textContent = "Detected Windows — v2.0.1 (unsigned installer).";

  const primaryLabel = $("[data-download-label]");
  if (primaryLabel) {
    if (platform === "mac") primaryLabel.textContent = "Download v2.0.1 for macOS";
    else if (platform === "windows") primaryLabel.textContent = "Download v2.0.1 for Windows";
  }
}
