...
function platformDetection() {
  ...
  if (platform === "mac" && sub) sub.textContent = "Detected macOS — v1.2.3 preview release.";
  if (platform === "windows" && sub) sub.textContent = "Detected Windows — v1.2.3 preview release.";

  const primaryLabel = $("[data-download-label]");
  if (primaryLabel) {
    if (platform === "mac") primaryLabel.textContent = "Download v1.2.3 for macOS";
    else if (platform === "windows") primaryLabel.textContent = "Download v1.2.3 for Windows";
  }
}
