...
| `replay_check_unfinished_run` | stable | ✓ | – | – | – | – | – | low | Returns the latest interrupted replay WAL row (`finished = false`) for crash recovery UI. |
| `replay_dismiss_unfinished_run` | stable | – | ✓ | – | – | – | – | low | Marks an interrupted replay run finished without undoing typed input. |
| `replay_undo` | stable | – | ✓ | – | – | – | ✓ | high | Best-effort undo for an interrupted replay: synthesizes backspaces for literal typed text; clicks/scrolls are skipped as non-reversible. |
