...
fallback when `title` is set, then opt-in template match when `template_png` is set.
Postconditions use AX verify or OCR text presence.
...
+ opt-in template match (`template_png`) + coordinate click. See
