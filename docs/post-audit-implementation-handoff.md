...
Before writing code, create or update an audit implementation tracker from the completed audit report. The tracker must identify the audit report source, the pull request that introduced the handoff, and the evidence required to close each finding.

Minimum tracker fields:

| Field | Requirement |
|---|---|
| Finding ID | Stable ID from the audit report. |
| Severity | Critical, High, Medium, Low, or Informational. |
| Source evidence | Audit section, file path, command output, screenshot, or release artifact that proves the issue exists. |
| Owner boundary | UI, IPC, planner, validator, executor, undo, audit, replay, release, website, or docs. |
| Fix PR | Pull request that resolves the finding. |
| Fix commit | Commit or commits that contain the fix. |
| Regression test | Automated test or documented verification command that fails before the fix and passes after it. |
| Documentation update | README, architecture, security, privacy, release, website, or other doc updated to match behavior. |
| Closure evidence | Reviewer-verifiable proof that the finding is resolved. |

Do not use this handoff as a substitute for the audit report. It is the execution plan for the report, not the evidence source.
...
2. **Implementation Handoff** — executes and resolves the audit findings; PR #163 introduced this handoff document and future implementation PRs should reference the completed audit report plus the relevant finding IDs.
