...
> **Architectural north star:** For five-year category and KEEP / REFACTOR / REPLACE / REMOVE decisions — **local transaction authority** for human-computer work (Transaction object, control vs execution plane, proof-carrying actions, resource versions / concurrency, capability leases, sagas, signed receipts, Ghost Execution Protocol, Cedar-bound policy) — see [`architecture-review-2031.md`](architecture-review-2031.md). “Kernel / Resource Graph / Action IR” remain implementation components under that category. Where this document’s ambition (marketplace, remote MCP, “OS for safe AI actions”) conflicts with that review or with the `AGENTS.md` wedge build order, prefer the architecture review and `AGENTS.md`. This file remains the MCP / Zones / approval-token vision sketch — as an **Intent Gateway / protocol client** story, not Ghost’s spinal architecture.
