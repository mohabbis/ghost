    ...
    fn from(err: std::io::Error) -> Self {
        match err.kind() {
            std::io::ErrorKind::NotFound => {
                Self::new(ErrorKind::FileSystem, "File not found").with_details(err.to_string())
            }
            std::io::ErrorKind::PermissionDenied => {
                Self::permission_denied("file system").with_details(err.to_string())
            }
            _ => {
                Self::new(ErrorKind::FileSystem, "File system error").with_details(err.to_string())
            }
        }
    }
}
...
impl From<anyhow::Error> for GhostError {
    fn from(err: anyhow::Error) -> Self {
        Self::new(ErrorKind::Internal, "Internal error").with_details(err.to_string())
    }
}
...
mod tests {
    ...
    fn test_error_display() {
        let err = GhostError::new(ErrorKind::Replay, "Test error").with_details("Some details");
