...
/**
 * Storing a recording trace, shared by every producer — the browser upload
 * form, the extension's ingest endpoint, and the worker's remote capture
 * browser — so they cannot drift on limits, sanitisation, or what gets audited.
 *
 * It lives in core rather than in the web app because the worker needs it too:
 * a cloud capture session ends inside the worker, and a second copy of "how a
 * trace becomes a Recording" is how the two would stop agreeing about what a
 * recording is.
 *
